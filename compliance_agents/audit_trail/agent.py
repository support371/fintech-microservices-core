...
class AuditTrailAgent:
    ...
    Usage:
        ...
        entry = agent.record_screening_decision(record, screening_result)
    ...
    def record_screening_decision(
        ...
        """
        Record the full screening context — rule results, LLM analysis,
        final risk score, and the watchlist version used.
        This is the core compliance audit entry.
        """
        ...
        event_type = event_map.get(result.final_decision, AuditEventType.RISK_SCORE_ASSIGNED)
        ...
        payload: Dict[str, Any] = {
            ...
            # Full transaction context for auditors
        ...
        if result.llm_analysis:
            payload["llm_analysis"] = {
                ...
                # Raw LLM response stored verbatim — never truncated
    ...
    def verify_chain_integrity(self) -> Dict[str, Any]:
        ...
        if not rows:
            return {"status": "empty", "verified_entries": 0, "broken_links": []}
        ...
        logger.info(
            f"[AuditTrailAgent] Chain integrity verified: "
            f"entries={len(rows)} broken={len(broken_links)} ok={integrity_ok}"
        )
        return {
    ...
    def _append(
        ...
        Computes sequence number, links prev_entry_hash, hashes this entry.
        """
        ...
        logger.info(
            f"[AuditTrailAgent] Entry appended: entry_id={entry.entry_id} "
            f"seq={entry.sequence_number} event={event_type.value}"
        )
        return entry
